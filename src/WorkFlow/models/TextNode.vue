...
import { toopTipText12, toopTipText13, toopTipText14, toopTipText15 } from '../data/tooltips'
    ...
    <div class="text-node-wrapper" ref="modeRef">
        <div :class="[openCard ? 'text-node-header' : '']">
            <div class="text-node-title">
                <el-button class="t-n-t-icon" :class="[openCard ? 't-n-is-active' : '']" :icon="RightIcon"
                    @click="openOrCloseCard" />
                <img src="../images/icon-text.png" alt="t-n-t-start-icon" class="t-n-t-start-icon" />
                <div class="t-n-t-text">
                    <el-input v-model="textNode.title.value" :placeholder="textNode.title.placeholder"
                        :disabled="textNode.title.disabled" :style="textNode.title.style" ref="titleRef" />
                </div>
                <ModelHeaderRight @testRun="testRun" @reName="reName" @deleteNode="deleteNode" />
        ...
        <div class="text-node-select-app" v-if="openCard">
            ...
            <div class="t-n-s-a-select">
                <el-select v-model="textNode.select.value" :placeholder="textNode.select.placeholder"
                    :disabled="textNode.select.disabled" :no-data-text="textNode.select.noDataText"
                    :style="textNode.select.style" @change="appSelectChange">
                    <el-option v-for="(oItem, opIndex) in textNode.select.options" :key="opIndex" :label="oItem.label"
                        :value="oItem.value">
                    </el-option>
                </el-select>
            </div>
            ...
            <el-collapse @change="collapseChange" v-model="openCollapseArr">
                <div class="text-node-content-item">
                    <el-collapse-item name="input" :disabled="true">
                        <template #title>
                            <el-button class="t-n-collapse-icon" :class="[openCollapse['input'] ? 't-n-is-active' : '']"
                                :icon="RightIcon" @click="clickCollapseItem('input')" />
                            <span class="t-n-c-text">输入</span>
                            <el-tooltip :offset="12" popper-class="tooltip-box-item-in-node" effect="dark"
                                :content="toopTipText12" placement="top">
                ...
                <div class="text-node-content-item">
                    <el-collapse-item name="string" :disabled="true">
                        <template #title>
                            <el-button class="t-n-collapse-icon" :class="[openCollapse['string'] ? 't-n-is-active' : '']"
                                :icon="RightIcon" @click="clickCollapseItem('string')" />
                            <span class="t-n-c-text">{{ textNode.string.text }}</span>
                            <el-popover v-if="textNode.select.value === '1'" placement="top" title="" :width="200"
                                trigger="hover" popper-class="t-n-c-r-popover">
                                <template #reference>
                            ...
                            <el-tooltip v-if="textNode.select.value === '2'" :offset="12"
                                popper-class="tooltip-box-item-in-node" effect="dark" :content="toopTipText14"
                                placement="top">
                                <el-icon class="t-n-c-header-icon t-n-c-header-icon-margin">
                                    <NoticeIcon />
                                </el-icon>
                            </el-tooltip>
                            <div class="t-n-c-header-right" v-if="textNode.select.value === '1'">
                                <el-button class="t-n-c-header-add-icon" :icon="SettingIcon" @click="settingString" />
                                <div class="t-n-c-setting-form" v-if="showSettingStringArea" ref="settingStringAreaRef">
                                    <div class="t-n-c-setting-form-title">
                                        数组连接符设置
                                    </div>
                                    ...
                                    <div>
                                        <div class="t-n-c-setting-form-subtitle">
                                            <el-tooltip :offset="12" popper-class="tooltip-box-item-in-node" effect="dark"
                                                :content="'连接符'" placement="top">
                                                <span>连接符</span>
                                            </el-tooltip>
                                            <el-tooltip :offset="12" popper-class="tooltip-box-item-in-node" effect="dark"
                                                :content="toopTipText15" placement="top">
                                                <el-icon class="t-n-c-header-icon ml4">
                                                    <NoticeIcon />
                                                </el-icon>
                                            </el-tooltip> 
                                            <span class="t-n-c-setting-form-subtitle-required">*</span>
                                        </div>
                                        <div class="t-n-c-setting-form-content">
111
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="t-n-c-result-content">
                            <el-input v-if="textNode.select.value === '1'" type="textarea"
                                ...
                                :rows="textNode.string.content.rows" :style="textNode.string.content.style" />
                            <el-select v-if="textNode.select.value === '2'" multiple
                                popper-class="t-n-c-result-content-select" v-model="textNode.splitString.value"
                                :placeholder="textNode.splitString.placeholder"
                                :disabled="textNode.splitString.disabled"
                                :no-data-text="textNode.splitString.noDataText" :style="textNode.splitString.style"
                                @change="splitSelectChange">
                                <el-option v-for="(oItem, opIndex) in textNode.splitString.options" :key="oItem.value"
                                    :label="oItem.label" :value="oItem.value" :disabled="oItem.type === 'others'">
                                    <CustomSelectOptionItem v-if="oItem.type === 'others'" @useItem="useItem" />
                                    <div class="t-n-c-select-option" v-else-if="oItem.type === 'custom'">
                                        <el-checkbox v-model="oItem.checkBoxOptions.selected"
                                            :disabled="oItem.checkBoxOptions.disabled"
                                            :style="oItem.checkBoxOptions.style">{{ oItem.checkBoxOptions.value }}</el-checkbox>
                                        <span class="t-n-c-s-o-label">{{ oItem.label }}</span>
                                        <div class="t-n-c-s-o-right">
                                            <el-tooltip :offset="12" popper-class="tooltip-box-item-delete"
                ...
                <div class="text-node-content-item">
                    <el-collapse-item name="output" :disabled="true">
                        <template #title>
                            <el-button class="t-n-collapse-icon" :class="[openCollapse['output'] ? 't-n-is-active' : '']"
                                :icon="RightIcon" @click="clickCollapseItem('output')" />
                            <span class="t-n-c-text">输出</span>
                            <el-tooltip :offset="12" popper-class="tooltip-box-item-in-node" effect="dark"
                                :content="toopTipText13" placement="top">
                                <el-icon class="t-n-c-header-icon t-n-c-header-icon-margin">
                                    <NoticeIcon />
                                </el-icon>
                            </el-tooltip>
                        ...
                        <div class="t-n-c-chat-outputlist">
                            <ModelTreeText :data="textNode.output.treeData" :openAll="true" :showBottomLine="false" />
                        </div>
                    </el-collapse-item>
                </div>
            </el-collapse>
